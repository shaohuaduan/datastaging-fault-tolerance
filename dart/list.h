...
//duan
#define list_for_each_entry_from_tail(pos, head, type, member)                          \
	for (pos = list_entry((head)->prev, type, member);      \
		 &pos->member !=( head);                                    \
		 pos = list_entry(pos->member.prev, type, member))

//duan
#define list_for_each_entry_with_next(pos, nextpos, head, type, member)                          \
	for (pos = list_entry((head)->next, type, member), nextpos = list_entry(pos->member.next, type, member) ;      \
		 &pos->member !=( head);                                    \
		 pos = list_entry(pos->member.next, type, member), nextpos = list_entry(pos->member.next, type, member) )
